...
public class AodPreviewActivity extends Activity {
                ...
                switch (tag) {
                    case "data:date":
                        if (view instanceof TextView) {
                            ((TextView) view).setText(new SimpleDateFormat("EEE, MMM d", Locale.getDefault()).format(new Date()));
                        }
                        break;
    ...
    private Object convertValueToType(Object value, Class<?> targetType, String key, View view) {
        if (targetType == float.class || targetType == Float.class) {
            ...
            if (value instanceof String) {
                ...
                try { return Color.parseColor(strValue); } catch (Exception e) {}
                if (key.equals("gravity") && view instanceof TextView) {
                    try { return parseGravity(strValue); } catch (Exception e) {}
                }
                try { return Integer.parseInt(strValue); } catch (Exception e) {}
            }
    ...
    private int parseGravity(String gravityString) {
        ...
        if (gravityString == null) return gravity;
    ...
    private RelativeLayout.LayoutParams createLayoutParams(JSONObject json, Map<String, Integer> idMap) {
        int width = json.optString("layout_width", "wrap_content").equalsIgnoreCase("match_parent") ? ViewGroup.LayoutParams.MATCH_PARENT : dpToPx(json.optInt("width", -2));
        int height = json.optString("layout_height", "wrap_content").equalsIgnoreCase("match_parent") ? ViewGroup.LayoutParams.MATCH_PARENT : dpToPx(json.optInt("height", -2));
        ...
        if (rules != null) {
            ...
            if (rules.has("below")) { Integer id = idMap.get(rules.optString("below")); if (id != null) params.addRule(RelativeLayout.BELOW, id); }
            if (rules.has("above")) { Integer id = idMap.get(rules.optString("above")); if (id != null) params.addRule(RelativeLayout.ABOVE, id); }
            if (rules.has("toRightOf")) { Integer id = idMap.get(rules.optString("toRightOf")); if (id != null) params.addRule(RelativeLayout.RIGHT_OF, id); }
            if (rules.has("toLeftOf")) { Integer id = idMap.get(rules.optString("toLeftOf")); if (id != null) params.addRule(RelativeLayout.LEFT_OF, id); }
        }
